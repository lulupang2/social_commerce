        ...
        items: [
          {
            ...
            reasonCodes: ["sport_match", "skill_level_match"],
            reasonText: "AI 추천: 실력대에 꼭 맞는 스키 장비입니다.",
            score: 95,
          },
          {
            listingId: "invented-listing-id-999", // Should be filtered out!
            reasonCodes: ["popular_item"],
            reasonText: "허위 상품",
            score: 100,
          },
        ],
    ...
    profileSports: [
      {
        ...
        sizePreferences: { skiLengthCm: 165 },
      },
    ],
    listings: [
      {
        ...
        details: { sport: "ski", skillLevel: "intermediate", lengthCm: 165 },
      },
      {
        ...
        details: { sport: "ski", skillLevel: "expert", lengthCm: 180 },
      },
    ],
...
Deno.test("CORS preflight (OPTIONS) returns 200 OK with CORS headers", async () => {
  const req = new Request("http://localhost/recommend-listings", { method: "OPTIONS" });
...
Deno.test("Non-POST method returns 400 invalid_request error envelope", async () => {
  const req = new Request("http://localhost/recommend-listings", { method: "GET" });
  const res = await handleRecommendListings(req, createMockDeps());
